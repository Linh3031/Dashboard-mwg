// Version 1.1 - Fix isNaN(Date) type error
// MODULE: SERVICES - DATA PROCESSING
// Chứa các hàm xử lý, chuẩn hóa, và phân tích cú pháp dữ liệu thô.

import { get } from 'svelte/store'; // <-- THÊM MỚI: Để đọc giá trị từ store
import { config } from './config.js';
import { utils } from './utils.js';

// <-- THÊM MỚI: Import các store Svelte thay vì appState -->
import {
    declarations,
    debugInfo,
    rawGioCongData,
...
export const dataProcessing = {
    ...
    // === START: CẬP NHẬT LOGIC ĐỌC KHAI BÁO (DÙNG STORES) ===
    getHinhThucXuatTinhDoanhThu: () => {
        const declarationData = get(declarations).hinhThucXuat; // <-- DÙNG get(declarations)
    ...
    getHinhThucXuatTraGop: () => {
        const declarationData = get(declarations).hinhThucXuatGop; // <-- DÙNG get(declarations)
        if (declarationData) return new Set(declarationData.split('\n').map(l => l.trim()).filter(Boolean));
        return new Set(config.DEFAULT_DATA.HINH_THUC_XUAT_TRA_GOP);
    },

    getHeSoQuyDoi: () => {
        const declarationData = get(declarations).heSoQuyDoi; // <-- DÙNG get(declarations)
        const heSoMap = {};
        if (declarationData) {
    ...
    normalizeData(rawData, fileType) {
        ...
        // <-- THAY ĐỔI: Dùng object cục bộ thay vì appState -->
        ...
        for (const key in mapping) {
            ...
            if (required) {
                ...
                newDebugInfo.required.push({ displayName, foundName: foundName || 'Không tìm thấy', status: status }); // Ghi vào object cục bộ
                if (!status) {
    ...
    parseLuyKePastedData: (text) => {
        // ... (Hàm này thuần túy, không thay đổi)
        const defaults = {
            mainKpis: {},
    ...
    parseCompetitionDataFromLuyKe: (text) => {
        // ... (Hàm này gần như thuần túy, chỉ ghi vào store competitionData)
        ...
        competitionData.set(results); // <-- THAY ĐỔI: Dùng store.set()
        return results;
    },

    /**
     * Cập nhật Bảng Ánh Xạ Tên Thi Đua trong store.
     *
     */
    updateCompetitionNameMappings(mainHeaders) {
        if (!mainHeaders || mainHeaders.length === 0) return;

        const oldMappings = get(competitionNameMappings) || {}; // <-- THAY ĐỔI: Dùng get()
        ...
        if (hasChanges) {
            competitionNameMappings.set(newMappings); // <-- THAY ĐỔI: Dùng store.set()
    ...
    processThiDuaNhanVienData(parsedData, luykeCompetitionData) {
        ...
        const $danhSachNhanVien = get(danhSachNhanVien); // <-- THAY ĐỔI: Dùng get()
        if ($danhSachNhanVien.length === 0) {
            newDebugInfo.status = 'Lỗi: Danh sách nhân viên (DSNV) chưa được tải lên.';
            debugInfo.update(current => ({ ...current, 'thiduanv-pasted': newDebugInfo })); // <-- THAY ĐỔI: Dùng update()
            return [];
        }
        ...
        const nameMappings = get(competitionNameMappings) || {}; // <-- THAY ĐỔI: Dùng get()
        ...
        const finalReport = [];
        const totalEmployeesInDSNV = $danhSachNhanVien.length; // <-- THAY ĐỔI
        const $employeeMaNVMap = get(employeeMaNVMap); // <-- THAY ĐỔI
        ...
        dataRows.forEach(row => {
            ...
            let employee;
            if (msnv) {
                employee = $employeeMaNVMap.get(msnv); // <-- THAY ĐỔI
            }
    ...
    processGioCongData: () => {
        ...
        const $rawGioCongData = get(rawGioCongData); // <-- THAY ĐỔI: Dùng get()
        const $employeeNameToMaNVMap = get(employeeNameToMaNVMap); // <-- THAY ĐỔI: Dùng get()
