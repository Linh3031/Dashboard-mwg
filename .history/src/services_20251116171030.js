// Version 1.3 - Add reactive employee map generation
...
// SỬA LỖI: Đường dẫn đúng là './services/dataProcessing.js' (camelCase)
...
import {
    ...
    masterReportData
} from './stores.js'; // Đã sửa đường dẫn này ở bước trước

// === BẮT ĐẦU THÊM MỚI (BƯỚC 2.1) ===
/**
 * Logic "Tự động lắng nghe".
 * Tự động chạy mỗi khi store 'danhSachNhanVien' bị thay đổi.
 * Nhiệm vụ: Cập nhật 2 stores tra cứu (employeeMaNVMap và employeeNameToMaNVMap).
 * Logic được lấy từ hàm 'services.updateEmployeeMaps' của dự án cũ.
 */
danhSachNhanVien.subscribe($danhSachNhanVien => {
    ...
    if (!$danhSachNhanVien || $danhSachNhanVien.length === 0) {
        console.warn("[Services] DSNV rỗng, không thể tạo maps.");
    } else {
        ...
        console.log(`[Services] Đã tự động cập nhật Employee Maps: ${newMaNVMap.size} MSNV, ${newNameMap.size} Tên.`);
    }
