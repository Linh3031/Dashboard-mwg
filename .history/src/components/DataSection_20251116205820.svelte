<script>
  ...
  // Xóa: import { danhSachNhanVien } from '../stores.js';
  // Xóa: import { dataProcessing } from '../services/dataProcessing.js';

  // Biến state cục bộ cho DSNV (Giữ nguyên)
  let dsnvFileName = "Chưa thêm file";
  let dsnvFileStatus = "";
  let isLoadingDSNV = false;

  // MỚI: Biến state cục bộ cho Giờ Công
  let giocongFileName = "Chưa thêm file";
  let giocongFileStatus = "";
  let isLoadingGioCong = false;

  /**
   * Xử lý khi người dùng chọn file DSNV (Đã tái cấu trúc).
   * Logic chính đã được chuyển sang dataService.
   */
  async function handleFileDSNV(event) {
    ...
    try {
      // Gọi service
      const result = await dataService.handleFileDSNV(file);
      dsnvFileStatus = result.message; // Cập nhật status từ kết quả
      
      if (result.success) {
        console.log("Store danhSachNhanVien đã được cập nhật bởi dataService.");
      } else {
        // Lỗi đã được service trả về trong result.message
      }
    } catch (err) {
      // Lỗi ngoài dự tính (ví dụ: service không tải được)
      console.error("Lỗi gọi dataService.handleFileDSNV:", err);
  ...
  /**
   * MỚI: Xử lý khi người dùng chọn file Giờ Công.
   */
  async function handleFileGioCong(event) {
    const file = event.target.files[0];
    if (!file) return;
    ...
    try {
      // Gọi service
      const result = await dataService.handleFileGioCong(file);
      giocongFileStatus = result.message; // Cập nhật status
      
      if (result.success) {
        console.log("Store rawGioCongData đã được cập nhật bởi dataService.");
      } else {
        // Lỗi đã được service trả về trong result.message
      }
    } catch (err) {
      console.error("Lỗi gọi dataService.handleFileGioCong:", err);
      giocongFileStatus = `❌ Lỗi: ${err.message}`;
    } finally {
      isLoadingGioCong = false;
      event.target.value = null;
    }
  }
  // === KẾT THÚC THAY ĐỔI ===
