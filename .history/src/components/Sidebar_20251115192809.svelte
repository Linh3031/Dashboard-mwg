<script>
  ...
  // Chúng ta sẽ thêm logic xử lý click (dispatch) sau.
  // Hiện tại chỉ tập trung hiển thị.
</script>
...
<nav id="sidebar" class="bg-white/80 backdrop-blur-sm shadow-lg fixed top-0 left-0 h-full z-30 p-3 flex flex-col justify-between">
    <div>
         <button class="nav-link flex items-center p-3 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold mb-4 w-full">
            <i data-feather="home"></i>
            <span class="menu-text">Hướng Dẫn & Góp Ý</span>
         </button>
        <ul class="space-y-2">
            <li>
                <button class="nav-link flex items-center p-3 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold w-full">
