// Version 1.6 - Hợp nhất logic v3.6 (Gốc) vào cấu trúc Svelte
...
export const dataProcessing = {
    ...
    getHeSoQuyDoi: () => {
        ...
        if (declarationData) {
            ...
            return Object.keys(heSoMap).length > 0 ? heSoMap : config.DEFAULT_DATA.HE_SO_QY_DOI;
        }
        return config.DEFAULT_DATA.HE_SO_QY_DOI;
    },
            ...
            foundHeaders.forEach((header, index) => {
                if (header) {
                    ...
                    if (upperKey.includes('KÊNH') || upperKey.includes('SIÊU THỊ') || upperKey.includes('NGÀNH HÀNG') || upperKey.includes('TỈNH') || upperKey.includes('BOSS')) {
                        ...
                        obj[header] = parseFloat(value.replace(/%|,/ag, '')) / 100 || 0;
