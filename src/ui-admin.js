// src/ui-admin.js
// Version 1.7 - Fix: Import from stores.js instead of state.js (Svelte Migration)
// MODULE: UI ADMIN
// Chứa các hàm render cho trang Quản trị (Khai báo).

import { get } from 'svelte/store'; // Dùng để lấy giá trị từ store
import { 
    sortState, 
...
} from './stores.js'; // Import các store thay vì appState
...
const renderUserStatsTable = (users) => {
    ...
    // SỬA LỖI: Lấy từ store sortState
    const currentSortState = get(sortState).user_stats || { key: 'lastLogin', direction: 'desc' };
    const { key, direction } = currentSortState;
...
const renderCompetitionNameMappingTable = () => {
    ...
    // SỬA LỖI: Lấy từ store competitionNameMappings
    const mappings = get(competitionNameMappings) || {};
    ...
    mappingEntries.forEach(([originalName, shortName], index) => {
        tableHTML += `
...
const renderCompetitionConfigUI = () => {
    ...
    // SỬA LỖI: Lấy từ store globalCompetitionConfigs
    const configs = get(globalCompetitionConfigs) || [];
...
const renderSpecialProgramConfigUI = () => {
    ...
    // SỬA LỖI: Lấy từ store globalSpecialPrograms
    const configs = get(globalSpecialPrograms) || [];
