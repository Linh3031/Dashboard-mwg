...
{#if isOpen}
    <div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-fade-in" on:click={close}>
       <div bind:this={captureNode} class="bg-white rounded-xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] mx-4" on:click|stopPropagation>
...
<style>
    ...
    /* --- GHI ĐÈ CSS ĐỂ CHỤP FULL NỘI DUNG (KHÔNG BỊ CẮT SCROLL) --- */
    ...
    :global(.capture-container .text-2xl),
    :global(.capture-container .text-base),
    :global(.capture-container .text-\[11px\]),
    :global(.capture-container .text-\[10px\]) {
