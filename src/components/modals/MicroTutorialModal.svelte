<script>
    ...
    // [PHẪU THUẬT LOGIC]: Tự động phục hồi dữ liệu từ LocalStorage khi F5 cực đoan
    onMount(() => {
        const cached = localStorage.getItem('cached_help_content');
        if (cached) {
            try {
                const parsedCache = JSON.parse(cached);
                // Kiểm tra xem dữ liệu Server có chứa Youtube URL hợp lệ nào không.
                // Tránh tình trạng Server nạp về cấu trúc rỗng đánh lừa App.
                const hasValidUrls = $helpContent && Object.values($helpContent).some(item => item && item.youtubeUrl && item.youtubeUrl.trim() !== '');
                
                if (!hasValidUrls) {
                    console.log("Dữ liệu Server hỏng/thiếu link. Khôi phục từ Local Cache...");
                    helpContent.set(parsedCache);
                }
            } catch(e) {
                console.error("Lỗi parse cache hướng dẫn", e);
            }
        }
    });
