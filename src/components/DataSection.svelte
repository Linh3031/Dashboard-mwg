<script>
  ...
  // === BẮT ĐẦU THAY ĐỔI (Tái cấu trúc Paste) ===
  ...
  // --- MỚI: Biến state cục bộ cho UI (Paste) ---
  ...
  // --- MỚI: Hàm xử lý sự kiện PASTE ---
  
  // Tạo một hàm debounce đơn giản
  let pasteTimer;
  function debounce(func, delay = 300) {
    return function(event) {
      clearTimeout(pasteTimer);
      // Giữ lại event.target.value vì Svelte có thể tái sử dụng event object
  ...
  // Hàm xử lý chung cho Paste (không async vì service chạy đồng bộ)
  function handlePasteInput(pastedText, serviceFunction, statusKey) {
    if (!pastedText || pastedText.trim().length < 10) {
        // Nếu xóa hết text, reset status
    ...
    try {
      const result = serviceFunction(pastedText);
  ...
  // Tạo các hàm debounced
  const handleLuykeInput = debounce(
    (text) => handlePasteInput(text, dataService.handleLuykePaste, 'luyke'), 
    500 // Chờ 500ms
  );
  
  const handleThiduaNVInput = debounce(
    (text) => handlePasteInput(text, dataService.handleThiduaNVPaste, 'thiduanv'), 
    500
  );
  
  const handleErpInput = debounce(
    (text) => handlePasteInput(text, dataService.handleErpPaste, 'thuongErp'), 
    500
  );
  
  const handleErpThangTruocInput = debounce(
    (text) => handlePasteInput(text, dataService.handleErpThangTruocPaste, 'thuongErpThangTruoc'), 
    500
  );
