...
if (sortType === 'current') { valA = currA; valB = currB; } 
                    ...
                    else if (sortType === 'percent') { valA = prevA ? (currA - prevA) / prevA : (currA > 0 ? 999999 : 0); valB = prevB ? (currB - prevB) / 
prevB : (currB > 0 ? 999999 : 0); }
    ...
    function toggleFilterDropdown(dimId) { if (openFilterId === dimId) openFilterId = null; else { openFilterId = dimId;
    filterSearchQuery = ''; } }
...
'+100%' : '0%';
    ...
    $: sortLabel = compareSortType === 'current' ?
'Số hiện tại' : (compareSortType === 'diff' ? 'Chênh lệch' : 'Tỷ lệ %');
</script>
        ...
        {#each allDimensions as dim (dim.id)}
            ...
            <div class="relative group">
                <div class="flex items-center rounded-md border {isActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-white'} hover:shadow-md transition-all">
                    ...
                    <button class="px-2 py-1.5 border-l border-gray-200 hover:bg-gray-100 {hasFilter ?
'text-blue-600' : 'text-gray-400'}" on:click|stopPropagation={() => toggleFilterDropdown(dim.id)}>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left">
            <thead class="bg-gray-100 text-gray-700 uppercase font-bold sticky top-0 z-10">
                <tr>
                    <th class="py-3 px-4 border-b border-r {isVelocityMode ?
'w-[300px] max-w-[300px]' : 'w-auto min-w-[200px]'}">
                        <div class="flex items-center gap-2">
                             ...
                             <div class="flex gap-1 ml-auto">
                               ...
                               <button on:click={() => expandAll(data)} class="p-1 hover:bg-gray-200 rounded" title="Mở rộng"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 13l-7 7-7-7m14-8l-7 7-7-7" /></svg></button>
                                <button on:click={collapseAll} class="p-1 hover:bg-gray-200 rounded" title="Thu gọn"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" 
stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" /></svg></button>
                            </div>
                        </div>
                    </th>
                    ...
                    <td class="py-3 px-4 text-right group relative">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            {#if isCompareMode && !isVelocityMode}
                                ...
                                <span class="text-xs {getDiffColor(diff)} bg-white/60 px-1 rounded">{diff > 0 ?
'+' : ''}{fmtQty(diff)}</span>
                            {/if}
                        </div>
                        {#if isCompareMode && !isVelocityMode}
                            ...
                            <div class="hidden group-hover:block absolute bottom-full right-0 mb-1 w-max px-3 py-2 bg-gray-800 text-white text-xs rounded shadow-lg z-50 pointer-events-none whitespace-nowrap font-normal">
                                ...
                                Chênh lệch: <b class="{diff >= 0 ? 'text-blue-300' : 'text-red-300'}">{diff > 0 ?
'+' : ''}{fmtQty(diff)} ({getPct(totalMetrics.quantity, totalMetrics.quantityCK)})</b>
                            </div>
                        {/if}
                    </td>
...
'+' : ''}{fmtRev(diff)}</span>
                        ...
                        {#if isCompareMode && !isVelocityMode}
                            ...
                            <div class="hidden group-hover:block absolute bottom-full right-0 mb-1 w-max px-3 py-2 bg-gray-800 text-white text-xs rounded shadow-lg z-50 pointer-events-none whitespace-nowrap font-normal">
                                ...
                                Chênh lệch: <b class="{diff >= 0 ? 'text-blue-300' : 'text-red-300'}">{diff > 0 ?
'+' : ''}{fmtRev(diff)} ({getPct(totalMetrics.revenue, totalMetrics.revenueCK)})</b>
                            </div>
                        {/if}
                    </td>
...
'+' : ''}{fmtRev(diff)}</span>
                        ...
                        {#if isCompareMode && !isVelocityMode}
                            ...
                            <div class="hidden group-hover:block absolute bottom-full right-0 mb-1 w-max px-3 py-2 bg-gray-800 text-white text-xs rounded shadow-lg z-50 pointer-events-none whitespace-nowrap font-normal">
                                ...
                                Chênh lệch: <b class="{diff >= 0 ? 'text-blue-300' : 'text-red-300'}">{diff > 0 ?
'+' : ''}{fmtRev(diff)} ({getPct(totalMetrics.revenueQD, totalMetrics.revenueQDCK)})</b>
                            </div>
                        {/if}
                    </td>
                    ...
                    {#if !isVelocityMode}
                        <td class="py-3 px-4 text-center">{totalMetrics.revenue > 0 ?
fmtPct((totalMetrics.revenueQD / totalMetrics.revenue) * 100) : '0%'}</td>
                    {/if}
...
'+' : ''}{fmtRev(diff)}</span>
                        ...
                        <td class="py-3 px-4 text-center bg-yellow-100">{totalMetrics.revenue > 0 ?
fmtPct((totalMetrics.revenueTraCham / totalMetrics.revenue) * 100) : '0%'}</td>
                    {/if}
                </tr>
            </thead>
            <tbody>
...
<style>
    ...
    :global(.capture-container .luyke-table-wrapper) {
        width: 1050px !important; min-width: 1050px !important;
