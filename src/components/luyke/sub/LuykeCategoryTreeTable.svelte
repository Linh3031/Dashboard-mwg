<script>
    ...
    // --- [PHẪU THUẬT LOGIC]: Chuyển đổi từ biến nội bộ chạy theo clock hệ thống thành Props nhận từ cha ---
    ...
    function sortTree(nodes, key, dir) {
        ...
        const sorted = [...nodes].sort((a, b) => {
            ...
            if (key === 'percentQD') {
                ...
                const currB = getProjected(b[baseKey] || 0);
                const prevB = b[`${baseKey}CK`] || 0;
    ...
    function toggleRow(id) { if (expandedRows.has(id)) expandedRows.delete(id); else expandedRows.add(id);
    expandedRows = expandedRows; }
    function expandAll(nodes) { if (!nodes) return;
    nodes.forEach(node => { expandedRows.add(node.id); if (node.children) expandAll(node.children); }); expandedRows = expandedRows;
    }
                    ...
                    {#if isColVisible('revenueQD')}
                        {#if isCompareMode && !isVelocityMode}
                         ...
                         {:else}
                            ...
                            {#if !isVelocityMode && !isCompareMode}
                                <td class="py-3 px-4 text-center bg-indigo-50/30 border-r">{totalMetrics.revenue > 0 ? fmtPct(((totalMetrics.revenueQD / totalMetrics.revenue) * 100) - 100) : '0%'}</td>
                            {/if}
                        {/if}
                    {/if}
