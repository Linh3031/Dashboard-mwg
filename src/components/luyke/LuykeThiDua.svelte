...
<div class="space-y-4">
  <div class="flex flex-wrap items-center justify-between gap-4">
    <div class="flex flex-wrap items-center gap-4"> 
      <h3 class="text-xl font-bold text-gray-800 uppercase">Thi đua lũy kế</h3> 
    ...
    <div id="luyke-competition-summary" class="text-sm font-semibold competition-summary-counter">
      (
  ...
  <div id="luyke-competition-infographic-container" class="mt-4">
    {#if sortedData.length === 0}
      <p class="text-gray-500 font-bold col-span-2">Vui lòng dán "Data lũy kế" ở tab Data để xem chi tiết.</p>
    {:else}
                ...
                {#each completed as item (item.name)}
                  ...
                  <div class="tdv-item-card">
                    ...
                    <div class="tdv-item-card__details">
                       <span>Lũy kế: <strong>{formatters.formatNumberOrDash(item.luyKe)}</strong></span>
                      <span>Target: <strong>{formatters.formatNumberOrDash(item.target)}</strong></span>
                      <span class="font-bold {dailyTarget > 0 ? 'text-red-600' : 'text-green-600'}">
                        Mục tiêu/ngày: <strong>{dailyTarget > 0 ? formatters.formatNumberOrDash(dailyTarget) : '-'}</strong>
                      </span>
                    </div>
                  </div>
                 {/each}
