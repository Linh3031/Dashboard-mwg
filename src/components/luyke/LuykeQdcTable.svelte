<script>
  ...
  $: sortedItems = (() => {
      ...
      selectedMacros.forEach(macroName => {
          const 
              config = macroConfigs.find(m => m.name === macroName);
          if (!config) return;
          ...
          if (childItems.length > 0) {
              const aggregatedItem = {
                  ...
                  dt: childItems.reduce((sum, i) => sum + (i.dt ||
                      0), 0),
           ...
           <div class="flex items-center gap-3">
               <div class="w-6 text-center font-bold text-gray-400 text-xs">#{index + 1}</div>
                <div class="flex-grow min-w-0 relative">
                   ...
                   <div class="w-full h-2 bg-gray-100 rounded-full overflow-hidden mb-1 shadow-inner relative z-0">
