<script>
  ...
  let localConfig = [];
  const BAR_COLORS = [
      'bg-red-500', 'bg-orange-500', 'bg-amber-500', 'bg-yellow-400', 'bg-lime-500',
      'bg-green-500', 'bg-emerald-500', 'bg-teal-500', 'bg-cyan-500', 'bg-sky-500',
      'bg-blue-500', 'bg-indigo-500', 'bg-violet-500', 'bg-fuchsia-500', 'bg-pink-500', 'bg-rose-500'
  ];
  ...
  $: if ($selectedWarehouse) { loadConfigForWarehouse($selectedWarehouse); } else { localConfig = allGroups.map(g => g.name); }

  async function loadConfigForWarehouse(kho) {
      try {
          const savedConfig = await datasyncService.loadQdcConfig(kho);
          if (savedConfig && Array.isArray(savedConfig)) { localConfig = savedConfig; } else { localConfig = allGroups.map(g => g.name); }
      } catch (e) { localConfig = allGroups.map(g => g.name); }
  }

  function toggleQdcSelection(name) {
      if (localConfig.includes(name)) localConfig = localConfig.filter(n => n !== name); else localConfig = [...localConfig, name];
      clearTimeout(saveTimer);
      saveTimer = setTimeout(() => { if ($selectedWarehouse) datasyncService.saveQdcConfig($selectedWarehouse, localConfig); }, 500);
  }

  function toggleAllVisibility(show) {
      localConfig = show ? allGroups.map(g => g.name) : [];
  ...
  function handleWindowClick(e) { if (isSettingsOpen && !e.target.closest('.filter-wrapper')) isSettingsOpen = false; }
...
<div class="luyke-widget h-full flex flex-col">
  <div class="luyke-widget-header">
    <div class="luyke-widget-title">
      <div class="p-1.5 bg-yellow-100 rounded text-yellow-600 mr-2"><i data-feather="star" class="w-4 h-4"></i></div>
      <span>TOP NHÓM HÀNG</span>
    </div>
    <div class="relative filter-wrapper">
        <button class="luyke-icon-btn {isSettingsOpen ? 'active' : ''}" on:click={() => isSettingsOpen = !isSettingsOpen} title="Chọn nhóm hàng"><i data-feather="filter" class="w-4 h-4"></i></button>
        {#if isSettingsOpen}
            <div class="filter-dropdown">
                <div class="filter-header"><input type="text" class="filter-search" placeholder="Tìm nhóm hàng..." bind:value={filterSearch} /></div>
                    ...
                    {:else}
                        {#each filterList as group}
                            ...
                            <div class="filter-item" on:click={() => toggleQdcSelection(group.name)}>
                                 ...
                                 {#if group.type === 'macro'}
                                    <label class="{isChecked ? 'font-bold text-teal-700' : 'text-teal-600 font-semibold'} flex items-center gap-1"><i data-feather="layers" class="w-3 h-3"></i> {group.name}</label>
                                 {:else}
  ...
  <div class="luyke-widget-body custom-scrollbar">
    {#if sortedItems.length === 0}
        <div class="flex flex-col items-center justify-center h-full text-gray-400 py-10"><p class="text-sm">Không có dữ liệu.</p></div>
    {:else}
      <div class="flex flex-col gap-0">
        {#each sortedItems as item, index (item.name)}
          ...
          <div class="py-2 border-b border-dashed border-gray-100 last:border-0 transition-colors px-1 {item.isMacro ? 'bg-teal-50/30 hover:bg-teal-50/50' : 'hover:bg-gray-50'}">
               ...
               <div class="w-6 text-center font-bold text-gray-400 text-xs">#{index + 1}</div>
                <div class="flex-grow min-w-0">
                   <div class="flex justify-between items-center mb-2">
                       ...
                       <span class="text-sm font-bold whitespace-nowrap text-gray-800">{formatters.formatRevenue(item.dtqd)}</span>
                   </div>
