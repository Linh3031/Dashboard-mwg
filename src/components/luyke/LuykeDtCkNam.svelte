<svelte:window on:click={() => { showSsgMonthsDropdown = false; showViewToggles = false; }} />
...
<script>
    ...
    // SSG CALENDAR COMPONENTS
    import SsgTargetSimulator from './dtcknam/SsgTargetSimulator.svelte';
    import SsgKpiBoard from './dtcknam/SsgKpiBoard.svelte';
    import SsgCalendarView from './dtcknam/SsgCalendarView.svelte';
    import { generateCalendarData } from './dtcknam/SsgCalendarLogic.js';
...
// Biến cho SSG Calendar (Mặc định 40%)
    ...
    $: maxTs2026 = data2026All.length > 0 ? data2026All.reduce((max, r) => {
    ...
    // NẠP NÃO BỘ LỊCH SSG VÀO ĐÂY
    $: if (viewMode === 'single' && selectedMonth && filteredSingleData.length > 0 && !isTransitioning) {
        const [mStr, yStr] = selectedMonth.split('/');
        const monthNum = parseInt(mStr, 10);
        const year26 = parseInt(yStr, 10) + 1; 
        
        const result = generateCalendarData(filteredSingleData, data2026All, monthNum, year26, targetGrowthPct);
        calendarGrid = result.calendarGrid;
        ssgKpiTotals = result.kpiTotals;
    }
</script>
...
{:else}
    ...
    <div class="flex flex-wrap items-center gap-4 mb-4">
        ...
        {#if viewMode === 'single' && availableMonths.length > 0}
            ...
            <div class="relative ml-2 animate-fade-in">
                <button class="flex items-center gap-2 px-3 py-1.5 border border-gray-300 rounded-md bg-white text-sm font-bold text-gray-600 hover:bg-gray-50 shadow-sm" on:click|stopPropagation={() => showViewToggles = !showViewToggles}>
                    <i data-feather="sliders" class="w-4 h-4 text-blue-500"></i> Hiển thị ▾
                </button>
                {#if showViewToggles}
                    <div class="absolute top-full mt-1 left-0 bg-white border border-gray-200 shadow-xl rounded-lg w-52 z-50 p-3" on:click|stopPropagation>
                        <p class="text-xs font-bold text-gray-400 uppercase mb-2">Bật/Tắt Giao diện</p>
                        <label class="flex items-center gap-2 text-sm font-bold text-gray-700 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
                            <input type="checkbox" bind:checked={showCalendar} class="rounded accent-blue-600 w-4 h-4"> Bảng Lịch Ngày
                        </label>
                        <label class="flex items-center gap-2 text-sm font-bold text-gray-700 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
                            <input type="checkbox" bind:checked={showCharts} class="rounded accent-blue-600 w-4 h-4"> Biểu đồ Tuần/Ngày
                        </label>
                        <label class="flex items-center gap-2 text-sm font-bold text-gray-700 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
                            <input type="checkbox" bind:checked={showTable} class="rounded accent-blue-600 w-4 h-4"> Bảng Danh mục
                        </label>
                    </div>
                {/if}
            </div>
    ...
    {:else}
        {#if viewMode === 'single'}
            <div class="animate-fade-in space-y-4">
                ...
                <SsgKpiBoard kpiTotals={ssgKpiTotals} />

                <div class="animate-fade-in mt-2 mb-4">
                    <SsgTargetSimulator bind:targetGrowthPct={targetGrowthPct} kpiTotals={ssgKpiTotals} />
                </div>

                {#if showCalendar}
                    <SsgCalendarView {calendarGrid} />
                {/if}
