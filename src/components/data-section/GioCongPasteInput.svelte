<script>
    ...
    import { rawGioCongData, fileSyncState, selectedWarehouse } from '../../stores.js';
    import { processGioCongPaste } from '../../services/data/gioCongPasteHandler.js';
    import { dataService } from '../../services/dataService.js';

    const RAW_TEXT_KEY = 'saved_giocong_paste_text';
    const BASE_KEY = 'saved_giocong';
    ...
    // Cơ chế kiểm tra đồng bộ có sẵn của app (syncHandler.js) lưu trạng thái dưới key khác nhau
    // tuỳ đang lọc "Tất cả kho" hay 1 kho cụ thể, trong khi ô này gộp nhiều kho nên luôn đọc key
    // gộp — dễ lệch. Dự phòng đọc thẳng meta lưu máy để không hiện trống dù dữ liệu vẫn còn nguyên.
    ...
    $: syncState = (!rawSyncState || !rawSyncState.metadata) && localMetaFallback
        ...
        : rawSyncState;
    $: uniqueWarehouses = [...new Set(($rawGioCongData || []).map(d => d.maKho).filter(Boolean).map(c => String(c).trim()))];
    ...
    function processText(text) {
        ...
        pasteTimer = setTimeout(async () => {
            try {
                const result = await processGioCongPaste(text);
                if (!result.success) {
    ...
    async function handleDownloadFromCloud() {
        ...
        try {
            ...
            // downloadFileFromCloud chỉ nạp dữ liệu vào store, không tự điền lại ô dán — tải luôn
            // nội dung gốc từ downloadURL để ô dán không trông như "chưa tải được".
...
<div class="data-input-group input-group--blue h-full">
    ...
    <div class="data-input-group__content flex flex-col flex-grow">
        <textarea
            ...
            placeholder="Dán dữ liệu Giờ công đã copy vào đây..."
        ...
        {#if unresolvedMaNV.length > 0}
            <div class="text-[11px] text-red-600 font-bold mt-1 flex items-center gap-1">
                <i data-feather="alert-circle" class="w-3 h-3"></i> MSNV chưa khớp DSNV (đã bỏ qua): {unresolvedMaNV.join(', ')}
            </div>
        {/if}
