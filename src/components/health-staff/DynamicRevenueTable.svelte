<script>
  ...
  // [FIX IMPORT] Đường dẫn tương đối
  ...
  // [FIX CRITICAL] Hàm lấy giá trị sort thông minh
  function getSortValue(item, key) {
      ...
      if (key === 'mainValue') val = item.mainValue;
          ...
          // 2. Xử lý cột động (Dynamic Columns)
          // Trường hợp key có dạng "ColID|Metric" (VD: Laptop|sl)
          if (key.includes('|')) {
              const [colId, metric] = key.split('|');
              if (item.cells && item.cells[colId]) {
                  // Map metric sang tên trường dữ liệu thực tế
                  ...
                  else val = item.cells[colId].value; // Mặc định lấy DT
              }
          } 
          // Trường hợp key là ID cột đơn thuần
      ...
      // 3. Ép kiểu số an toàn (Chống lỗi NaN)
      let finalVal = 0;
      if (typeof val === 'string') {
          // Xóa tất cả ký tự không phải số/dấu chấm/dấu trừ (VD: "1,200" -> 1200)
  ...
  function handleSort(event) {
      ...
      // Đổi hướng nếu sort lại cùng 1 cột
