<script>
    ...
    export let config = {};
    // Cấu hình bảng (từ Admin/User)
    export let reportData = [];
    // Dữ liệu nhân viên (Master Data)
    ...
    function getValueColor(cell, targetObj) {
        if (cell.config.type === 'PERCENT') {
            ...
            if (targetVal > 0) {
                ...
                'text-blue-600 font-bold' : 'text-red-600 font-bold bg-red-50';
            }
            return 'text-red-600';
            // Mặc định % là màu đỏ (hoặc cam)
        }
                ...
                <thead class="bg-gray-50 text-xs uppercase text-gray-600 font-bold sticky top-0 z-20 shadow-sm">
                    <tr>
                        ...
                        {#each config.columns as col (col.id)}
                            {@const bgClass = col.color ?
                            `bg-${col.color}-50` : 'bg-gray-50'}
                            {@const textClass = col.color ?
                            `text-${col.color}-800` : 'text-gray-700'}
                            <SortableTh 
                                key={col.id} 
                                label={col.header} 
