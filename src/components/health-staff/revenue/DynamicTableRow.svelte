...
<tr class="hover:bg-blue-50/50 transition-colors group">
    ...
    {#each columns as col}
        ...
        {#if metrics.sl}
            <td class="px-2 py-2 text-right border-r border-gray-200 border-b text-gray-600" title={cell?.tooltip}>{formatters.formatNumber(cell?.sl)}</td>
        {/if}
        {#if metrics.dt}
            <td class="px-2 py-2 text-right border-r border-gray-200 border-b font-medium text-gray-800" title={cell?.tooltip}>{formatters.formatRevenue(cell?.dt)}</td>
        {/if}
        {#if metrics.dtqd}
            <td class="px-2 py-2 text-right border-r border-gray-200 border-b font-medium text-purple-600" title={cell?.tooltip}>{formatters.formatRevenue(cell?.dtqd)}</td>
        {/if}
    {/each}
</tr>