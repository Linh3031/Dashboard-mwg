...
<thead class="uppercase text-[11px] font-bold sticky top-0 z-10 shadow-sm align-middle">
    <tr>
        ...
        {#each visibleColumns as col}
            {@const isTargetGroup = col.group === 'mt'}
            {@const mtBgClass = isTargetGroup ? '!bg-emerald-100 !text-emerald-800' : ''}
            
            <th class="px-1 py-2 transition select-none text-center cursor-pointer hover:brightness-95 {revenueTableUtils.getGroupHeaderClass(col)} {mtBgClass} overflow-hidden" style="width: 90px; min-width: 90px; max-width: 90px;"
                ...
                <div class="flex flex-row items-center justify-center w-full h-full gap-1">
                    ...
                    {#if sortKey === col.key}
                        <span class="{isTargetGroup ? 'text-emerald-600' : 'text-blue-600'} font-black flex-shrink-0 text-[10px] mt-0.5">{sortDirection === 'asc' ? '▲' : '▼'}</span>
                    {/if}
                </div>
            </th>
        {/each}
