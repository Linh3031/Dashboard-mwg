    ...
    <details class="group" open> 
        ...
        <div class="p-6 border-t border-slate-100 bg-slate-50/50">
            ...
            <div class="flex space-x-2 mb-6 border-b border-gray-200 pb-1">
                ...
                <button class="px-4 py-2 font-medium text-sm rounded-t-lg transition-colors {activeTab === 'slider' ? 'bg-white text-pink-600 border-x border-t border-gray-200' : 'text-gray-500 hover:text-gray-700'}" on:click={() => activeTab = 'slider'}>Slide Ảnh (URL)</button>
                            ...
                            <div class="divide-y divide-slate-100">
                                {#each localConfig.sliderImages as img, index}
                                    <div class="flex items-start gap-4 p-4 hover:bg-slate-50 transition-colors">
                                        <div class="w-32 h-20 bg-slate-200 rounded-lg border overflow-hidden flex-shrink-0 relative flex items-center justify-center">
                                            {#if img.url}
                                                <img src={img.url} alt="Thumbnail" class="w-full h-full object-cover" on:error={(e) => e.target.src = 'https://placehold.co/128x80?text=Lỗi+Ảnh'} />
                                            {:else}
                                                <i data-feather="image" class="text-slate-400 w-8 h-8"></i>
                                            {/if}
                                        </div>
                                        <div class="flex-grow flex flex-col gap-3">
                                            <div>
                                                <label class="block text-xs font-bold text-slate-500 mb-1">Đường dẫn ảnh (URL)</label>
                                                <input type="text" bind:value={img.url} class="w-full p-2 border border-slate-200 rounded text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="https://...">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-bold text-slate-500 mb-1">Tiêu đề (Tùy chọn)</label>
                                                <input type="text" bind:value={img.title} class="w-full p-2 border border-slate-200 rounded text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="VD: Sức khỏe siêu thị">
                                            </div>
                                        </div>
                                        <button on:click={() => removeSlide(index)} class="p-2 mt-4 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><i data-feather="trash-2" class="w-5 h-5"></i></button>
                                    </div>
                                {/each}
                            </div>
                        </div>
