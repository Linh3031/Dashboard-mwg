<script>
  ...
  import { 
      ...
      // [CODEGENESIS] Import Store KPI Mới
      kpiStore 
  } from '../../stores.js';
  ...
  async function loadGoals(kho) {
      ...
      // [CODEGENESIS] Đồng bộ ngược từ dữ liệu tải về vào kpiStore để bảng hiển thị đúng ngay khi mở
      updateKpiStore();
  }
  ...
  // [FIX] Cập nhật kpiStore khi nhập liệu
  function updateKpiStore() {
      kpiStore.update(store => ({
          ...store,
          // Cập nhật settings toàn cục để bảng tô màu ngay lập tức
          globalSettings: {
  ...
  function saveLuyke() {
      ...
      // 1. Lưu vào kho cũ (để giữ logic đồng bộ cloud cũ)
      settingsService.saveLuykeGoalForWarehouse(localSelectedWarehouse, currentLuykeGoals);
      
      // 2. [CODEGENESIS] Bắn tín hiệu sang kho mới để tô màu Realtime
      updateKpiStore();
  }
  ...
  <div class="space-y-8">
          ...
          <div class="space-y-5 animate-fade-in">
              ...
              <details class="group border rounded-lg overflow-hidden" open>
                  <summary class="p-3 text-sm font-bold cursor-pointer bg-gray-50 hover:bg-gray-100 flex justify-between items-center select-none">
                      Mục tiêu khai thác (%) <span class="transform group-open:rotate-180 transition-transform">▼</span>
                  </summary>
      ...
      {#if activeTab === 'daily'}
          <div class="space-y-5 animate-fade-in">
              ...
              <details class="group border rounded-lg overflow-hidden" open>
                  <summary class="p-3 text-sm font-bold cursor-pointer bg-gray-50 hover:bg-gray-100 flex justify-between items-center select-none">
                      Mục tiêu tỷ lệ Realtime (%) <span class="transform group-open:rotate-180 transition-transform">▼</span>
                  </summary>
                  <div class="p-4 bg-white border-t grid grid-cols-2 gap-4">
                      {#each percentageInputs as input}
                          <div>
