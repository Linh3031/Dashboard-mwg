...
// Version 2.3 - Fix silent failure on save (Add notify for permission denied)
import { get } from 'svelte/store';
import { doc, setDoc, getDoc, getDocs, collection } from "firebase/firestore"; 
import { firebaseStore, isAdmin, homeConfig } from '../stores.js';
...
export const adminService = {
    ...
    async saveHomeConfig(configData) {
        const db = getDB();
        // [FIX] Thêm thông báo lỗi cụ thể thay vì return im lặng
        ...
        try {
            ...
            homeConfig.set(configData); // Cập nhật store local
    ...
    async loadHomeConfig() {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                console.log("[admin.service] Đã tải Home Config từ Cloud.");
            }
        } catch (error) {
    ...
    async saveCompetitionNameMappings(mappings) {
        const db = getDB();
        if (!db) return; // Silent return here is okay for auto-save, or we can log
