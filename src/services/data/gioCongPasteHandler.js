// src/services/data/gioCongPasteHandler.js
// Xử lý dữ liệu "Giờ công" dán tay (thay cho Excel bị công ty chặn xuất). Dùng LẠI đúng cache
...
export async function processGioCongPaste(rawText) {
    const mapping = FILE_MAPPING[BASE_KEY];

    try {
        const { results, unresolvedMaNV, error } = parseGioCongPasted(rawText);

        if (error) return { success: false, message: error };
        if (results.length === 0) {
            const msg = unresolvedMaNV.length > 0
                ? `Không khớp được MSNV nào với DSNV: ${unresolvedMaNV.join(', ')}`
                : `Không tìm thấy dòng dữ liệu hợp lệ. Kiểm tra lại dữ liệu đã dán.`;
            return { success: false, message: msg };
        }

        // Gộp trên dữ liệu GỐC trong cache offline làm nền — không dùng store đang hiển thị, để
        // không xoá mất dữ liệu của các kho khác (giống hệt cách fileHandler.js xử lý Excel).
        ...
        const validWarehouses = Array.from(uploadedKhoSet).filter(Boolean);
        ...
        try {
            ...
            for (const wh of validWarehouses) {
                const metadata = {
                    ...
                    fileName: 'du_lieu_dan_giocong.txt',
                    fileType: 'text_giocong_paste',
            ...
            const successMsg = `✓ Đã trích xuất ${results.length} dòng - ${validWarehouses.length} kho` + (cloudWarning ? ` — ⚠️ ${cloudWarning}` : '');
            updateSyncState(BASE_KEY, 'synced', successMsg, { updatedAt: new Date(now), timestamp: now });
        } catch (cloudErr) {
            console.error('Lỗi đồng bộ Cloud (Giờ công dán):', cloudErr);
